"""Hyperparameter tuning and final model selection for weekly regression."""
...
TUNING_ROOT = Path("data/results/tuning/weekly/regression")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune weekly regression models")
...
def main() -> None:
    ...
    notes_path.write_text(
        "\n".join(
            [
                f"# Weekly Regression Final Model ({timestamp})",
                ...
                "",
                "Tuning details saved under data/results/tuning/weekly/regression/",
            ]
        ),
        encoding="utf-8",
    )
    ...
    print(f"Saved weekly regression model -> {final_dir}")
