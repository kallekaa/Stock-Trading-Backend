"""Hyperparameter tuning and final model selection for daily regression."""
...
TUNING_ROOT = Path("data/results/tuning/daily/regression")
FINAL_MODEL_DIR = Path("data/results/final_models/daily/regression")
DROP_COLUMNS = ["date", "ticker"]


SEARCH_SPACE: Dict[str, List[Dict[str, float | int]]] = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune daily regression models")
...
def evaluate_param_set(
    ...
    for train_idx, val_idx in splitter.split(X):
        ...
        metrics = metrics_utils.regression_metrics(y_val, preds)
...
def main() -> None:
    ...
    notes_path.write_text(
        "\n".join(
            [
                f"# Daily Regression Final Model ({timestamp})",
                ...
                "",
                "Tuning details saved under data/results/tuning/daily/regression/",
            ]
        ),
        encoding="utf-8",
    )
