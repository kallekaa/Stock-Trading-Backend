"""Hyperparameter tuning and final model selection for daily time-series forecasting."""
...
TUNING_ROOT = Path("data/results/tuning/daily/time_series")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune daily forecasting models")
...
def main() -> None:
    ...
    notes_path.write_text(
        "\n".join(
            [
                f"# Daily Time Series Final Model ({timestamp})",
                f"Horizon: {horizon}",
                ...
                "",
                "Tuning details saved under data/results/tuning/daily/time_series/",
            ]
        ),
        encoding="utf-8",
    )
    ...
    print(f"Saved final time-series model -> {final_dir}")
