"""Run baseline time-series forecasting experiments on weekly data."""
...
FREQUENCY = "weekly"
TASK = "time_series"
RESULTS_ROOT = Path("data/results/experiments/weekly/time_series")
DROP_COLUMNS = ["date", "ticker"]
TARGET_TEMPLATE = "target_forecast_close_t+{horizon}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run baseline time-series models on weekly data")
...
def run_experiment(args: argparse.Namespace, config: dict) -> None:
    ...
    for horizon in horizons:
        ...
        for spec in models:
            ...
            print(f"Completed weekly horizon {horizon} -> {run_dir}")
