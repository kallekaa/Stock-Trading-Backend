"""Hyperparameter tuning and final model selection for daily classification."""
...
TUNING_ROOT = Path("data/results/tuning/daily/classification")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune daily classification models")
...
def main() -> None:
    ...
    notes_path.write_text(
        "\n".join(
            [
                f"# Daily Classification Final Model ({timestamp})",
                ...
                "",
                "Tuning details saved under data/results/tuning/daily/classification/",
            ]
        ),
        encoding="utf-8",
    )
    ...
    print(f"Saved final classification model -> {final_dir}")
